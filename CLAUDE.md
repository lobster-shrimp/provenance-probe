...
2. **Fleet detection — wedge SHIPPED (v0.24.1)** — `provenance-probe fleet-scan` (no-egress, read-only host forensics): discovers agent-CLI `base_url`s in config files + env, resolves localhost gateways to their real upstream via gateway-config parsing (the OmniRoute localhost blind-spot fix), and classifies against an operator allowlist + bundled `corpus.py` attribution as sub-CONFIRMED pointers (never a measured verdict). New pure `provenance_probe/gateways.py` keeps the no-egress boundary structural. Shipped: engine + CLI (#69); **osquery delivery** — `--sqlite` sink + `--print {launchd,systemd,cron,osquery-atc}` (#71); the **fleet host-attribution zero-FP control** wired into the hermetic eval gate (#72); the **prevention posture** — `--print allowlist-template` starter + `docs/fleet-posture.md`, report headlines `allowlist holding: N sanctioned, M drifted` not a rogue list (#73). B-phase remaining: the trust-store watch (transparent-MITM case, with an enterprise-trusted-root baseline), Tier-2 network-egress capture (pcap/JA3, loopback fan-out), a signed public provider-attribution registry (generated *from* `corpus.py`), Intune/Tanium delivery recipes, and the Windows collector.
