...
2. **Fleet detection — wedge SHIPPED (#69)** — `provenance-probe fleet-scan` (no-egress, read-only host forensics): discovers agent-CLI `base_url`s in config files + env, resolves localhost gateways to their real upstream via gateway-config parsing (the OmniRoute localhost blind-spot fix), and classifies against an operator allowlist + bundled `corpus.py` attribution as sub-CONFIRMED pointers (never a measured verdict). New pure `provenance_probe/gateways.py` keeps the no-egress boundary structural. B-phase remaining: osquery/Intune/Tanium delivery recipe, wiring the widened zero-FP cases into the `eval/` gate, the trust-store watch (transparent-MITM case), Tier-2 network capture, a signed public registry (generated *from* `corpus.py`), and reference-allowlist/posture docs.
